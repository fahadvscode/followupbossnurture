...
export function LeadConversationPanel({
  ...
  return (
    <div className={cn('space-y-4', embedded ? '' : 'max-w-2xl mx-auto')}>
      {!embedded && (
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-foreground">{contactName || 'Conversation'}</h2>
              {isAi && conversation ? (
      ...
      {isAi && conversation && conversation.status === 'active' && !isHumanTakeover && (
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 rounded-lg border border-dashed border-border bg-card p-3">
          <p className="text-sm text-muted">
            <strong className="text-foreground">Start fresh</strong> — clear the AI’s memory for this thread.
          </p>
          <button
            type="button"
      ...
      {isAi && isHumanTakeover && (
        <div className="flex items-center justify-between rounded-lg border border-blue-300 bg-blue-50 p-3">
          ...
          <button
            ...
            disabled={sending}
            className="flex items-center gap-1 text-xs font-medium text-blue-700 border border-blue-300 rounded-lg px-3 py-1.5 hover:bg-blue-100 disabled:opacity-50"
          >
      ...
      <div className="rounded-xl border border-border bg-card px-4 min-h-[280px]">
        {loading ? (
      ...
      {isAi && isEnded && !isEscalated && (
        <div className="rounded-lg border border-border bg-card p-3 text-sm text-muted">
          AI conversation ended ({conversation?.status.replace(/_/g, ' ')}). Use Start fresh to re-engage.
        </div>
      )}
