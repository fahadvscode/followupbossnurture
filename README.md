...
- **Reply alerts (optional):** when a lead replies by SMS, the app can email you the reply text plus a button that opens the conversation. Requires SMTP (above). Set **`REPLY_NOTIFY_EMAIL`** to one or more recipient addresses (comma-separated); if unset it falls back to `EMAIL_FROM`/`SMTP_USER`. The deep link uses `NEXT_PUBLIC_BASE_URL` (or `VERCEL_URL` on Vercel) to build the URL. Replies are also logged to the FUB timeline as before.
